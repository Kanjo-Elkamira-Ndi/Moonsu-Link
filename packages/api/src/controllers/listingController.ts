...
export const createListing = async (req: Request, res: Response) => {
    try {
        const { user_id, crop_id, quantity_kg, price, town, image_url } = req.body;
        if (!user_id || !crop_id || !quantity_kg || !price || !town) {
            throw new AppError("Missing required fields", 400);
        }
        const listing = await listingService.createListing({ user_id, crop_id, quantity_kg, price, town, image_url });
